            ...
            <div class="w-full">
                ...
                <div class="mb-8">
                    <!-- <Link :href="route('siswa.assignment.index')"
                        class="inline-flex items-center gap-1.5 text-sm text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-white mb-3 transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                        </svg>
                        Back to Assignments
                    </Link> -->
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">Edit Your Assignment
                    </h1>
                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Update the details of your submitted
                        assignment.</p>
                </div>
                ...
                <form ref="assignmentForm" @submit.prevent="submitAssignment" class="space-y-5">
                    ...
                    <div
                        ...
                        <div v-if="existingFilePath && fileType !== 'none'"
                            ...
                            <p class="text-xs text-amber-700 dark:text-amber-400">
                                Current attachment: <span class="font-semibold">{{ existingFileName }}</span>.
                                Upload a new file or enter a new link to replace it, or switch to "None" to remove it.
                            </p>
                        </div>
                        ...
                        <div v-if="fileType === 'link'" class="space-y-1.5">
                            ...
                            <input type="url" v-model="form.link" maxlength="2048"
                                class="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-3.5 py-2.5 text-sm text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition"
                                placeholder="https://youtube.com/..." />
                        </div>
                        <p v-if="fileType === 'none'" class="text-sm text-gray-400 dark:text-gray-500 italic">
                            {{ existingFilePath ? 'Current attachment will be removed on save.' : 'No attachment will be added.' }}
                        </p>
                    </div>
...
const props = defineProps({
    assignment: Object,  // existing assignment data
    teachers: Array,     // [{ id, nama_lengkap }]
    subjects: Array,     // [{ id, mapel, guru_id }]
})

// Detect existing attachment type
...
/**
 * When teacher changes, reset subject ONLY if current mapel doesn't
 * belong to the newly selected teacher (prevents clearing on mount).
 */
...
const submitAssignment = () => {
    ...
    // Normalize line endings from mobile (\r\n or \r → \n)
