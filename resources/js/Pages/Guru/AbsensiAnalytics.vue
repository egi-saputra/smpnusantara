...
function selectKelas(kelasId) {
    loading.value = true
    router.get(route('guru.absensi.analytics'), { kelas_id: kelasId }, {
...
function changeKelas() {
    router.get(route('guru.absensi.analytics'), {}, { preserveScroll: false })
}
...
function applyFilter() {
    ...
    if (filterMode.value === 'bulan') {
        loading.value = true
        router.get(route('guru.absensi.analytics'), {
            ...base,
            mode: 'bulan',
...
const siswa = computed(() =>
    props.analytics?.siswa ?? []
)

const rekapKelas = computed(() =>
    props.analytics?.rekap_kelas ?? null
)
...
// Siswa urut dari tertinggi ke terendah kehadiran
...
// Stacked bar harian
function renderBar() {
    const canvas = document.getElementById('analyticsBarDaily')
    ...
    barInstance = new Chart(canvas, {
        ...
        data: {
            ...
            datasets: [
                {
                    label: 'Hadir',
                    data: trendHarian.map(d => d.hadir),
                ...
                {
                    label: 'Sakit',
                    data: trendHarian.map(d => d.sakit),
                ...
                {
                    label: 'Alpha',
                    data: trendHarian.map(d => d.alpha),
...
function renderTrend() {
    const canvas = document.getElementById('analyticsTrendLine')
...
// Re-render trend saat tab berubah
watch(trendTab, async () => {
    await nextTick()
    renderTrend()
})

// Re-render semua saat data baru masuk (navigasi Inertia)
...
<template>
    ...
    <MenuLayout>
        <div class="min-h-screen bg-slate-100 dark:bg-slate-950 text-slate-900 dark:text-slate-100">
            ...
            <header>
                <div class="mx-auto py-4 px-2 flex items-center justify-between gap-4">
                    <div class="flex items-center gap-3">
                        <div
                            class="w-9 h-9 rounded-xl bg-blue-500/20 text-blue-400 flex items-center justify-center text-lg flex-shrink-0">
                            📊
                        </div>
                        <div>
                            <div class="text-sm font-bold dark:text-gray-200 text-gray-700 leading-tight">Attendance
                                Analytics</div>
            ...
            <main class="mx-auto space-y-5 pb-10">

                <!-- ══ STEP 1: PILIH KELAS ════════════════════════════════ -->
                <template v-if="!kelas">
                    <div class="text-center py-4">
                        <span
                            class="inline-block px-3 py-1 rounded-full bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-300 text-[11px] font-bold uppercase tracking-widest mb-3">
                            Langkah 1
                        </span>
                        <h2 class="text-2xl font-extrabold text-slate-800 dark:text-white">Pilih kelas untuk dianalisis
                        </h2>
                        <p class="mt-1.5 text-sm text-slate-500 dark:text-slate-400">Lihat insight kehadiran, tren, dan
                            siswa yang perlu perhatian.</p>
                    </div>
                ...
                <template v-else>
                    ...
                    <template v-if="hasData">
                        ...
                        <!-- Alert siswa di bawah threshold -->
                        <div v-if="bottomSiswa.length"
                            ...
                            <p class="text-sm text-rose-700 dark:text-rose-300">
                                <strong>{{ bottomSiswa.length }} siswa</strong> memiliki tingkat kehadiran di bawah {{
                                    threshold }}%.
                                Segera tindaklanjuti!
                            </p>
                        </div>

                        <!-- ── Tabel: Top siswa + Siswa perlu perhatian ── -->
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            ...
                            <div
                                ...
                                <div>
                                    ...
                                    <div v-if="!topSiswa.length" class="px-5 py-10 text-center text-slate-400 text-sm">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-5 gap-4">
                            ...
                            <div
                                ...
                                <div style="position:relative;height:200px;">
                                    <canvas id="analyticsDonut" role="img"
                                        aria-label="Pie chart distribusi status kehadiran kelas.">
                            ...
                            <div
                                ...
                                <div class="flex items-center justify-between mb-3">
                                    ...
                                    <div class="flex flex-wrap gap-2">
                                        <span
                                            v-for="(color, label) in { Hadir: '#639922', Sakit: '#BA7517', Izin: '#378ADD', Alpha: '#E24B4A' }"
                                            :key="label"
                                ...
                                <div style="position:relative;height:220px;">
                                    <canvas id="analyticsBarDaily" role="img"
                        ...
                        <div
                            ...
                            <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
                                <p
                                    class="text-[11px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500">
                                    Tren Rata-rata Kehadiran Kelas
                                </p>
                            ...
                            <div v-if="trendData.length" style="position:relative;height:200px;">
                                <canvas id="analyticsTrendLine" role="img"
                                    aria-label="Line chart tren rata-rata kehadiran kelas.">
