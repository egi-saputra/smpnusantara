...
const formatFormat = (f) =>
    f === 'Berurutan' ? 'Sequential' : f === 'Acak' ? 'Shuffle' : f;
    ...
    <MenuLayout>
        ...
        <div class="mb-8 grid grid-cols-2 sm:grid-cols-3 gap-3">
            <div class="rounded-xl border border-gray-200 dark:border-slate-800
                ...
                <p class="text-xs text-gray-400 dark:text-slate-500 mb-1">Quiz Token</p>
            ...
            <div class="rounded-xl border border-gray-200 dark:border-slate-800
                ...
                <p class="font-semibold text-gray-800 dark:text-slate-200">
                    {{ formatFormat(soal.tipe_soal) }}
                </p>
            </div>

            <div class="rounded-xl border border-gray-200 dark:border-slate-800
                        bg-white dark:bg-slate-900 p-4 shadow-sm">
                <p class="text-xs text-gray-400 dark:text-slate-500 mb-1">Duration</p>
                <p class="font-semibold text-gray-800 dark:text-slate-200">
                    {{ soal.waktu }} minutes
                </p>
            </div>
            ...
            <div class="rounded-xl border border-gray-200 dark:border-slate-800
                        bg-white dark:bg-slate-900 p-4 shadow-sm">
                <p class="text-xs text-gray-400 dark:text-slate-500 mb-1">Subject</p>
                <p class="font-semibold text-gray-800 dark:text-slate-200">
                    {{ soal.mapel?.mapel ?? '-' }}
                </p>
            </div>
            ...
            <div class="rounded-xl border border-gray-200 dark:border-slate-800
                        bg-white dark:bg-slate-900 p-4 shadow-sm">
                <p class="text-xs text-gray-400 dark:text-slate-500 mb-1">Class Unit</p>
        ...
        <div class="mb-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div class="flex items-center gap-2">
                <Link href="/guru/soal" class="hidden md:inline-flex items-center gap-1.5 rounded-lg
                ...
                <span class="text-sm text-gray-500 dark:text-slate-400">
                    {{ soal.bank_soal?.length ?? 0 }} questions
                </span>
            </div>
        ...
        <div v-if="!soal.bank_soal || soal.bank_soal.length === 0" class="flex flex-col items-center justify-center rounded-2xl
            ...
            <p class="font-medium text-gray-500 dark:text-slate-400">No questions yet</p>
            <p class="text-sm text-gray-400 dark:text-slate-500 mt-1">
                Click "Add Question" to start adding questions.
            </p>
        </div>
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div v-for="(item, index) in soal.bank_soal" :key="item.id" class="group rounded-2xl border border-gray-200 dark:border-slate-800
                        ...
                        <template v-for="key in ['a', 'b', 'c', 'd', 'e']" :key="key">
                            ...
                            <div v-else class="flex items-center gap-2 rounded-lg px-2.5 py-1.5
                                ...
                                <span class="text-xs text-gray-400 dark:text-slate-500 italic">
                                    Option {{ key.toUpperCase() }} not filled
                                </span>
                            </div>
                        </template>
                    </div>

                    <!-- Correct Answer box (Essay / fallback PG) -->
                    <div class="rounded-lg px-3 py-2" :class="getCorrectAnswerText(item)
                        ? 'bg-green-50 dark:bg-green-500/10 border border-green-200 dark:border-green-500/20'
                        : 'bg-gray-50 dark:bg-slate-800/40 border border-gray-200 dark:border-slate-700'">
                        <p class="text-xs font-semibold mb-0.5" :class="getCorrectAnswerText(item)
                ...
                <div class="flex gap-2 px-4 py-3 border-t border-gray-100 dark:border-slate-800">
                    <Link :href="`/guru/bank-soal/${item.id}/edit`" class="flex-1 inline-flex items-center justify-center gap-1.5
