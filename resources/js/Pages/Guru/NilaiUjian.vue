        ...
        <div class="nilai-page">
            ...
            <header class="page-header">
                <div class="header-left">
                    ...
                    <div>
                        ...
                        <p class="page-subtitle">Rekap & Evaluasi Hasil Ujian Siswa</p>
                    </div>
                </div>
            ...
            <template v-else-if="sortedRekap.length > 0">
                    ...
                    <table class="data-table">
                        ...
                        <tbody>
                            <tr v-for="(item, i) in paginatedRekap" :key="`${item.user_id}-${item.soal_id}`"
                                class="data-row">
                                <td class="col-no text-muted">{{ (currentPage - 1) * perPage + i + 1 }}</td>
                                <td class="col-text cell-name">{{ item.nama_lengkap }}</td>
                                <td class="col-text"><span class="badge badge-kelas">{{ item.nama_kelas }}</span></td>
                                <td class="col-text">{{ item.nama_mapel }}</td>
                                <td class="col-num">{{ item.total_soal }}</td>
                                <td class="col-num">{{ item.dijawab }}</td>
                                <!-- <td class="col-num"><span class="num-warn">{{ item.tidak_dijawab }}</span></td> -->
                                <td class="col-num"><span class="num-success">{{ item.total_benar }}</span></td>
                                <!-- <td class="col-num"><span class="num-danger">{{ item.salah }}</span></td> -->
                                <td class="col-num"><span class="nilai-badge">{{ item.total_nilai }}</span></td>
                                <td class="col-center">
                ...
                <div class="card-list md:hidden">
                    <div v-for="(item, i) in paginatedRekap" :key="`mob-${item.user_id}-${item.soal_id}`"
                        class="nilai-card">
                        ...
                        <div class="card-nilai-row">
                            <span class="card-nilai-label">Nilai Akhir</span>
...
import {
    ...
    ArrowUpIcon, ArrowDownIcon, XMarkIcon,
...
/**
 * Backend sudah return flat data — kita hanya perlu
 * filter status Selesai di sini agar konsisten dengan versi Proktor.
 */
...
onMounted(async () => {
    try {
        ...
        // Support kedua format: array langsung atau { data: [...] }
...
const exportExcel = async () => {
    ...
    sheet.eachRow({ includeEmpty: false }, (row, rn) => {
        ...
        row.eachCell((cell, colNumber) => {
            ...
            cell.alignment = {
                vertical: 'middle',
                horizontal: colNumber === 2 ? 'left' : 'center', // kolom Nama = left
            };
        });
    });
...
.card-stats {
    @apply grid grid-cols-3 gap-2 border-t border-gray-100 dark:border-gray-700 pt-3;
}
